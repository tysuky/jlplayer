...
(function () {
    ...
    function jlPlayer(jlPlayerID) {
        ...
        function construct() {
            ...
            // Eventos de seeker
            [jlPlayerProgress, jlPlayerSliderVol].map((evt) => { evt.addEventListener('mousedown', startDrag) })
            ...
            ['webkitfullscreenchange', 'mozfullscreenchange', 'fullscreenchange'].map((evt) => { document.addEventListener(evt, checkFullscren) })

            // Eventos do vídeo
            handler(jlPlayerVideo, 'canplay', canplay, !0);
            handler(jlPlayerVideo, 'timeupdate', updateTimer, !0);
            ['waiting', 'canplay', 'playing', 'play', 'loadstart', 'seeked'].map((evt) => { jlPlayerVideo.addEventListener(evt, videoLoader) })
        ...
        // Controla o volume a partir do slider
